...
class ProductController extends BaseVoyagerBaseController
{
    ...
    public function getattribute($id)
    {
        ...
        return view('vendor.voyager.products.attribute',compact('productAttributes','product', 'attributes','id'));

    }

    public function postattribute(Request $request, $id)
    {
        $this->authorize('edit', app('App\Models\Product'));
        $product = Product::find($id);
        if($request->has('attributeID') && $request->input('attributeID') != 0){
            $model = ProductAttribute::find($request->input('attributeID'));
            $old_model = $model;
            $model->price = $request->input('productAttributePrice');
            $model->sale_price = $request->input('sale_price');
            $model->default = $request->input('default');
            $model->quantity = $request->input('productAttributeQuantity');
            $model->save();

            $this->updateProductStock($product,$old_model->quantity,$model->quantity);
            $this->defaultAttribute($model->id, $id);

            return redirect()->route('admin.product.attribute.detail',$product->id)
            ->with('message', 'Cập nhật thành công!');
        }
        else if ($request->has('attributeValue')) {
    ...
    public function deteleAttribute()
    {
        if(request()->has('id_detele') && request()->has('pa_detele'))
        {            
            $productID = request()->get('id_detele');
            $productAttributeID = request()->get('pa_detele');
            $product = Product::find($productID);
            $pa = $product->attributes()->where('id','=',$productAttributeID)->first();
            ...
            $this->updateStock($product, $qty);
            $this->defaultAttribute($productAttributeID,$productID);
            // request()->session()->flash('message', 'Xóa thành công!');
            return redirect()->route('admin.product.attribute.detail', $productID)->with('message', 'Xóa thành công!');
        }else{
    ...
    /*
    *** only one default in product
    */
    public function defaultAttribute($id,$productID){
        $model = ProductAttribute::where('id','=',$id)->where('product_id','=',$productID)->get();
        foreach($model as $item){
            $item->default = 0;
            $item->save();
        }

        if(!$model->where('default','=',1)->count())
        {
            $m = ProductAttribute::where('id','=',$id)->where('product_id','=',$productID)->orderby('created_at', 'ASC')->first();
            $m->default = 1;
            $m->save();
        }
    }
    ...
    *** update product stock
    */
    public function updateProductStock(Product $product,$old_qty,$new_qty)
    {
        $quantity = 0;
        if($new_qty && $old_qty){
            $quantity = ($product->stock - $old_qty) + $new_qty;
        }
    ...
    public function getProductAttributeValueByAttributeId($id)
    {
        $this->authorize('read', app('App\Models\Product'));
...
//            return redirect()->route('voyager.products.edit',[$product->id, 'combination' => 1])
//                ->withErrors($errors);
//        }
        ...
        $hasDefault = $product->attributes()->get()->where('default', 1)->count();
        ...
        $productAttribute = $product->saveProductAttributes(
            new ProductAttribute(compact('quantity', 'price', 'sale_price', 'default'))
        );

        // save the combinations
        return collect($attributeValues)->each(function ($attributeValueId) use ($product, $productAttribute) {
            $attribute = AttributeValue::find($attributeValueId);
            return $product->saveCombination($productAttribute, $attribute);
        })->count();
    }

    private function updateProductAttributeValue(Request $request, Product $product): bool
    {
        $fields = $request->only(
            'productAttributeQuantity',
            'productAttributePrice',
        ...
        if($request->has('attibuteID')){
            $model = ProductAttribute::find($fields['attributeID'])->get();
            $model->quantity = $fields['productAttributeQuantity'];
            $model->price = $fields['productAttributePrice'];
            $model->sale_price = $fields['sale_price'];
            $model->default = $fields['default'];
            $model->save();
            return 1;
        }else
            return 0;
    ...
    private function updateStock(Product $product, $qty)
    {
        $product->stock = $qty;
        $product->save();
    }
